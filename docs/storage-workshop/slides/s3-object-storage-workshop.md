...
1. backend start ได้
2. เรียก `POST /storage/presign-upload`
...
async function uploadAvatar(file: File) {
  const presignRes = await fetch('http://localhost:3001/storage/presign-upload', {
    method: 'POST',
...
<div class="grid grid-cols-2 gap-8 mt-8">
  <div class="rounded-lg bg-white p-6 shadow border">
    ...
    <div class="mt-3 text-lg text-slate-600">http://localhost:3000</div>
  </div>
  <div class="rounded-lg bg-white p-6 shadow border">
